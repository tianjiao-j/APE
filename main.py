...
def APE_T(cfg, cache_keys, cache_values, val_features, val_labels, test_features, test_labels, clip_weights, clip_model,
          train_loader_F):
    feat_dim, cate_num = clip_weights.shape
...
def main():
    ...
    if not path.exists(result_file):
        ...
        cfg_cp = yaml.load(open(result_file, 'r'), Loader=yaml.Loader)
    cfg_cp['shotss'].append(cfg['shots'])
    ...
    cfg_cp['zs_accs'].append(zs_acc)
    cfg_cp['accs'].append(acc)
    ...
    if cfg['dataset'] == 'imagenet':
        ...
        train_loader_F = build_data_loader(data_source=dataset.train_x, batch_size=256, tfm=train_tranform,
                                           is_train=True, shuffle=True)
    ft_acc = APE_T(cfg, cache_keys, cache_values, val_features, val_labels, test_features, test_labels, clip_weights,
                   clip_model, train_loader_F)
    cfg_cp['ft_accs'].append(ft_acc)

    with open(result_file, 'w') as outfile:
        yaml.dump(cfg_cp, outfile, default_flow_style=None)
